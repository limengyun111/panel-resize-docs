...
class DragElement {
  ...
  constructor(dragEle, customDragMove, direction = 'horizontal', customCursor) {
    ...
    // 修改过
    this.isHorizontal ? this.dragEle.style.width = '10px' : this.dragEle.style.height = '10px';
  ...
  setDragElePos = (mouseCurX, mouseCurY) => {
    ...
    this.onMouseMove && this.onMouseMove(mouseCurX, mouseCurY, offSetX, offSetY);
...
export default class PanelResize {
  ...
  // 已修改
  constructor(parentEle, sizeData, direction = 'horizontal', customCursor, autoSaveId) {
    if (PanelResize.panelResizeEleHistory.has(parentEle)) {
      return [...PanelResize.panelResizeEleHistory].find(item => item === parentEle);
    }

    PanelResize.panelResizeEleHistory.add(parentEle);
  ...
  init() {
    ...
    const panelsFlexRatio = this.distributeSizes();
    ...
    this.panelFlexInParent = [...this.panelsEle.map(panel => parseFloat(panel.style.flexGrow) / this.totalFlex)];
    this.handlesEle.forEach(handle => {
      const handler = new DragElement(handle, true, this.direction, this.customCursor);
      ...
      handler.onMouseMove = this.handleMove;
  ...
  // 设置面板宽度
  adjustFlexOnResize = () => {
    const newTotalFlex = this.isHorizontal ? this.getTotalWidthFlex() : this.getTotalHeightFlex();
    this.panelsEle.forEach((panel, index) => {
      const curGrow = this.panelFlexInParent[index] * this.totalFlex;
      const nextGrow = curGrow / this.totalFlex * newTotalFlex;
      panel.style.flexGrow = nextGrow;
      this.panelsFlex[index] = nextGrow;
      this.startPanelsFlex[index] = nextGrow;
    });

    this.totalFlex = newTotalFlex;
    this.panelFlexInParent = [...this.panelsEle.map(panel => parseFloat(panel.style.flexGrow) / this.totalFlex)];
  ...
  getPanelRatioRanges = (panel) => {
    ...
    return {
      // 修改过
      minSize: (data?.minSize || this.DEFAULT_PANEL_MIN_RATIO),
      defaultSize: (data?.defaultSize || this.DEFAULT_PANEL_RATIO),
      maxSize: (data?.maxSize || this.DEFAULT_PANEL_MAX_RATIO)
    }
  }

  distributeSizes() {
    // 计算defaultSize总和
    const items = this.panelsEle.map(panel => this.getPanelRatioRanges(panel));
    ...
    if (savedLayout && this.autoSaveId) {
      ...
      if (Date.now() > savedLayoutItem.expiry) {
        localStorage.removeItem(`layout-${this.autoSaveId}`); // 清除过期数据
      }
      return savedLayoutItem?.value.map(it => it * 100)
    }

    const totalDefault = items.reduce((sum, item) => sum + item.defaultSize, 0);
    const scale = 100 / totalDefault;

    // 第一轮分配：按比例计算并四舍五入
    let result = items.map(item => Math.round(item.defaultSize * scale));
    ...
    if (remaining !== 0) {
      ...
      const adjustments = items.map(item =>
        remaining * (item.defaultSize / totalDefault)
      );
  ...
  isPanelMove(offset, panel, direction) {
    ...
    const panels = isLeftOrTop
      ? this.panelsEle.slice(0, idx + 1)  // 左/上：取当前及左侧/上方面板
      : this.panelsEle.slice(idx);
    return panels.reduce((res, p, i) => {
      const { minSize, maxSize } = this.getPanelRatioRanges(p);
      const minFlex = minSize / 100 * this.totalFlex;
      const maxFlex = maxSize / 100 * this.totalFlex;
      const curFlex = this.startPanelsFlex[isLeftOrTop ? i : idx + i];
  ...
  getPanelAllRatioFlex(pIndex) {
    ...
    const minFlex = (minSize / 100 * this.totalFlex).toFixed(1);
    const maxFlex = maxSize / 100 * this.totalFlex;
    const curFlex = this.startPanelsFlex[pIndex];
  ...
  handleMove = (x, y) => {
    ...
    this.panelsFlex = this.panelsEle.map(panel => parseFloat(panel.style.flexGrow));
    this.panelFlexInParent = [...this.panelsEle.map(panel => parseFloat(panel.style.flexGrow) / this.totalFlex)];
  }
  setLayout(layouts) {
